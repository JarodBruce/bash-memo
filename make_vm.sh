...
for vm in "${vm_list[@]}"; do
  IFS=',' read -r vm_name vm_ip <<< "$vm"
  echo "Create VM: $vm_name with IP: $vm_ip"
  (
    qm create ${vmid} --name "$vm_name" --core ${PVE_CORE} --memory ${PVE_MEMORY} --net0 virtio,bridge=vmbr8
    qm importdisk ${vmid}  ubuntu-24.04.img local-lvm
    qm set ${vmid} --scsihw virtio-scsi-pci --scsi0 local-lvm:vm-${vmid}-disk-0
    qm resize ${vmid} scsi0 +${PVE_VOLUME}G
    qm set ${vmid} --ide2 local-lvm:cloudinit
    qm set ${vmid} --boot order=scsi0
    qm set ${vmid} --serial0 socket --serial1 socket --vga virtio
    qm set ${vmid} --agent enabled=1
    qm set ${vmid} --ipconfig0 ip="$vm_ip",gw=${DEFAULT_GATEWAY}
